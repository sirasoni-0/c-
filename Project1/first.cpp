...
어째서 헤더파일 안에 그냥 함수를 구현하거나, 전역 변수를 선언하고 그냥 참조하면 안될까?
ㄴ 헤더파일을 단순히 참조하면 파일을 개별적으로 컴파일 하는건 문제가 없지만, 
   헤더파일을 여러 소스파일에서 참조할 때, 함수 구현이 중복되면 링크 에러가 발생함.
   ...
   2025.8.14
   스태틱, 외부변수애 대해 배움'
   사실 지역변수와 전역변수는 변수의 스코프(영향이 미치는 범위)에 대한 구분이다.
   스태틱 변수는 함수의 스코프를 선언된 곳(전역이면 파일 내부, 지역이면 함수 내부)에서만 작동하지만 저장된 값은 사라지는게 아닌 메모리에 저장된다.(버튼 누른 횟구 같은거)
   그렇기 때문에, 외부에서 변수에 접근하는 것을 차단할 때 사용하거나, 함수 내부에서 값을 처음에만 초기화 할때, 데이터를 누적하여 저장해야할 때 사용한다. 
   여러 파일에서 전역 변수를 공유하고 싶을 때, 단순히 헤더에 전역 변수를 정의하면 중복 정의 오류가 발생한다.
   따라서 외부 변수(extern)를 선언해서 한 파일에 정의하고 다른 파일에서 참조한다.
   

   2025.8.23
   포인터는 변수가 저장된 메모리의 주소를 담는 변수이다.
   포인터 선언은 자료형 + '*' + 변수명 의 형태이다.
   int main(void)
	{
			int i = 10;
			int* pi = NULL;  포인터 선언
			pi = &i; 포인터에 변수의 주소를 넣음
			printf("%u %u\n", pi, &i);
			return 0;
	}
	 pi 와 &i는 전달받은 주소가 가르키는 값이 아닌	전달받은 주소를 출력함.(값은 랜덤)
	 만약 전달받은 값을 출력하고 싶다면 아래 코드가 맞음
	 printf("%d", *pi);
	 pi = 주소값 저장
	 *pi = 주소에 있는 실제 값 참조

	 포인터를 선언할때 꼭 null로 초기화를 해야됨.
	 왜냐하면 초기에 쓰레기값이 있는데, 어느 메모리 주소인지 몰라서 엉뚱한 곳에 접근할 수 있음.

	 포인터 타입은 변수 타입과 일치해야된다.
	 그렇지 않을경우 메모리침범(buffer overflow)가 생겨서, 오류가 발생한다.

	 포인터에 증감 연산을 하면, 정수 그대로 더해지는게 아닌, 자료형의 크기 단위로 증가한다.
	 ex pi+1을 하면, 포인터가 pi의 주소를 int로 해석하기 때문에, 기존 주소 + 4r 가 된다.
	 ...
	 int arr[10] = {};  빈 10칸짜리 int배열 선언(

	 *(arr + 1) = 10; == arr[1]; 10l 배열 2번째 칸에 10 삽입.
*/
